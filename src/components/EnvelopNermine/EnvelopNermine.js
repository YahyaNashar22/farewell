...
import style from "./EnvelopNermine.module.css";

function EnvelopNermine({isT04, isFadi, isOmar, isNermine, setIsNermine}) {
  ...
  return (
    <section className={`
                          ${style.wrapper}
                          ${isOmar || isFadi || isT04 ? style.hiddenEnvelop:""}
                          ${isNermine?style.singleView:""}
                          `}>
      <div className={style.envelopeWrapper}>
        ...
        >
          <p className={!isAnimated?style.name:style.hiddenName}>Nermine</p>
      ...
      <div className={style.reset}>
        <button
          ...
          onClick={() => {setIsAnimated(!isAnimated);setIsNermine(!isNermine)}}
        >
          {!isAnimated?
          'Open'
        :
